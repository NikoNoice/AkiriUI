...
function Library.Utils:AddColor(option)
	...
	local function SimulateHUE()
		...
		option.hue = 1 * xRatio
		local color = Color3.fromHSV(option.hue, 1, 1)
	...
	local function Calculate(hue, inputColor)
		local xRatio, s, yRatio

		if inputColor then
			xRatio, s, yRatio = inputColor:ToHSV()
		...
		SchemaI.BackgroundColor3 = option.color
		HUEI.Position = UDim2.new(typeof(xRatio) == "number" and xRatio or 0, -8 * (typeof(xRatio) == "number" and xRatio or 0), -0.5, 0)
	...
	function option:SetValue(hue, color)
		Calculate(hue, color)
	end
	...
	ColorpickerBTN.MouseButton1Click:Connect(function()
		if ColorpickerValue.Value then
			...
			do
				for i, v in pairs(game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Skid"):GetDescendants()) do
...
function Library:New(option)
	...
	function Library:Close()
		...
		local skidCanvas = ScreenGui
		local tweenCount = 0

		local function tweenElement(element, properties)
			local tween = game.TweenService:Create(element, TweenInfo.new(0.4), properties)
		...
		for _, element in pairs(skidCanvas:GetDescendants()) do
			local transparencyData = {}
			if element:IsA("TextLabel") then
				transparencyData = {TextTransparency = element.TextTransparency, BackgroundTransparency = element.BackgroundTransparency, TextStrokeTransparency = element.TextStrokeTransparency}
				coroutine.wrap(tweenElement)(element, {TextTransparency = 1, BackgroundTransparency = 1, TextStrokeTransparency = 1})
			elseif element:IsA("TextBox") then
				transparencyData = {TextTransparency = element.TextTransparency, BackgroundTransparency = element.BackgroundTransparency}
				coroutine.wrap(tweenElement)(element, {TextTransparency = 1, BackgroundTransparency = 1})
			elseif element:IsA("ImageLabel") then
				transparencyData = {ImageTransparency = element.ImageTransparency}
				coroutine.wrap(tweenElement)(element, {ImageTransparency = 1})
			elseif element:IsA("TextButton") then
				transparencyData = {TextTransparency = element.TextTransparency, BackgroundTransparency = element.BackgroundTransparency}
				coroutine.wrap(tweenElement)(element, {TextTransparency = 1, BackgroundTransparency = 1})
			elseif element:IsA("Frame") or element:IsA("ScrollingFrame") then
				transparencyData = {BackgroundTransparency = element.BackgroundTransparency}
				coroutine.wrap(tweenElement)(element, {BackgroundTransparency = 1})
			elseif element:IsA("ImageButton") then
				transparencyData = {ImageTransparency = element.ImageTransparency}
				coroutine.wrap(tweenElement)(element, {ImageTransparency = 1})
			elseif element:IsA("UIStroke") then
				transparencyData = {Transparency = element.Transparency}
				coroutine.wrap(tweenElement)(element, {Transparency = 1})
			end
			transparencyTable[element] = transparencyData
		end
	end
	...
	function Library:Open()
		...
		for element, transparencyData in pairs(transparencyTable) do
			if element:IsA("TextLabel") then
				coroutine.wrap(tweenElement)(element, {TextTransparency = transparencyData.TextTransparency, BackgroundTransparency = transparencyData.BackgroundTransparency, TextStrokeTransparency = transparencyData.TextStrokeTransparency})
			elseif element:IsA("TextBox") then
				coroutine.wrap(tweenElement)(element, {TextTransparency = transparencyData.TextTransparency, BackgroundTransparency = transparencyData.BackgroundTransparency})
			elseif element:IsA("ImageLabel") then
				coroutine.wrap(tweenElement)(element, {ImageTransparency = transparencyData.ImageTransparency})
			elseif element:IsA("TextButton") then
				coroutine.wrap(tweenElement)(element, {TextTransparency = transparencyData.TextTransparency, BackgroundTransparency = transparencyData.BackgroundTransparency})
			elseif element:IsA("Frame") or element:IsA("ScrollingFrame") then
				coroutine.wrap(tweenElement)(element, {BackgroundTransparency = transparencyData.BackgroundTransparency})
			elseif element:IsA("ImageButton") then
				coroutine.wrap(tweenElement)(element, {ImageTransparency = transparencyData.ImageTransparency})
			elseif element:IsA("UIStroke") then
				coroutine.wrap(tweenElement)(element, {Transparency = transparencyData.Transparency})
			end
		end
	end
	...
	Title.Text = option.title -- Remove RichText tags
	Title.TextColor3 = Color3.fromRGB(255, 255, 255)
	Title.TextSize = 24.000
	Title.RichText = false -- Remove RichText property
	...
	Avatar.Image = game:GetService("Players"):GetUserThumbnailAsync(game:GetService("Players").LocalPlayer.UserId, Enum.ThumbnailType.AvatarThumbnail, Enum.ThumbnailSize.Size100x100)
	...
	User.Text = game:GetService("Players").LocalPlayer.DisplayName
		...
		function ColumnHandler:AddColumn()
			...
			function SectionHandler:AddSection(title)
				...
				function Utilities:AddButton(option)
					...
					Button.BackgroundColor3 = Color3.fromRGB(21, 21, 27)
					--[[Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
					Button.BorderSizePixel = 0--]]
					Button.Size = UDim2.new(0, 215, 0, 15)
					Button.Text = option.text
					Button.TextColor3 = Color3.fromRGB(119, 119, 119)
				...
				function Utilities:AddLabel(option)
					option = type(option) == "table" and option or {}
					option.text = option.text or "Text Label"
					option.flag = option.flag or option.text
					...
					TextLabel.Size = UDim2.new(0, 223, 0, 15)
					TextLabel.TextSize = 8.000
					TextLabel.BackgroundTransparency = 1
					TextLabel.Parent = Section
					TextLabel.TextColor3 = Color3.new(0.466667, 0.466667, 0.466667)
				...
				function Utilities:AddSlider(option)
					...
					local function Modulate()
						...
						local sliderOutX = SliderOut.AbsolutePosition.X
						local sliderOutWidth = SliderOut.AbsoluteSize.X
						local toggleFrameWidth = ToggleFrameInner.AbsoluteSize.X

						local newPosition = UDim2.new(
							0,
							math.clamp(mouseX - sliderOutX - toggleFrameWidth / 2, 0, sliderOutWidth - toggleFrameWidth),
							-2.7,
							0
						)
						...
						local newSize = UDim2.new(
							0,
							math.clamp(mouseX - sliderOutX, 0, sliderOutWidth),
							0,
							2
						)

						option.value = math.clamp(
							(ToggleFrameInner.Position.X.Offset / (sliderOutWidth - toggleFrameWidth)) * (option.max - option.min) + option.min,
							option.min,
							option.max
						)
						...
						ToggleFrameInner.Position = newPosition
						SliderIn.Size = newSize
					end

					local function Calculate(number)
						local newPosition = UDim2.new(
							0,
							math.clamp(((number - option.min) / (option.max - option.min)) * (SliderOut.AbsoluteSize.X - ToggleFrameInner.AbsoluteSize.X), 0, SliderOut.AbsoluteSize.X - ToggleFrameInner.AbsoluteSize.X),
							-2.7,
							0
						)
						...
						local newSize = UDim2.new(
							0,
							math.clamp((number - option.min) / (option.max - option.min) * SliderOut.AbsoluteSize.X, 0, SliderOut.AbsoluteSize.X),
							0,
							2
						)
					...
					SliderSelection.MouseButton1Down:Connect(function()
						...
						releaseConnection = game.UserInputService.InputEnded:Connect(function(input, gameProcessedEvent)
							if input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					TextBox.FocusLost:Connect(function()
						if tonumber(TextBox.Text) then
							TextBox.Text = tostring(math.clamp(tonumber(TextBox.Text), option.min, option.max))
						end
					end)

					function option:SetValue(input)
						if tonumber(input) then
							Calculate(tonumber(math.clamp(tonumber(input), option.min, option.max)))
						end
					end

					function option:AddColor(colorOption)
						colorOption = type(colorOption) == "table" and colorOption or {}
						colorOption.color = colorOption.color or Color3.fromRGB(255, 255, 255)
						colorOption.parent = Slider
						colorOption.transparency = colorOption.transparency or 0
						colorOption.flag = colorOption.flag or "New Color"
						colorOption.callback = colorOption.callback or function() end
						colorOption.utilitytype = "color"

						return Library.Utils:AddColor(colorOption)
					end
						...
						for i, v in pairs(option.values) do
							...
							function option:SetValue(value)
								if not option.multi then
									local selectedValue = value and tostring(value) or ""

									for _, v in pairs(TextButton.Parent:GetChildren()) do
									...
									for _, v in pairs(TextButton.Parent:GetChildren()) do
										if v:IsA("TextButton") and v.Text == selectedValue then
											v.TextColor3 = Library.Theme.Highlighted
											TextButton.Text = selectedValue
											DDSelected.Text = selectedValue
								...
								elseif option.multi and type(value) == "table" then
									SelectedValues = SortTable(option.values, value)
									DDSelected.Text = tostring(table.concat(SelectedValues, ", "))

									for _, v in pairs(TextButton.Parent:GetChildren()) do
										if v:IsA("TextButton") then
											if table.find(SelectedValues, v.Text) then
												v.TextColor3 = Library.Theme.Highlighted
											else
												v.TextColor3 = Library.Theme.NotFocused
											end
									...
									warn("Invalid value provided to SetValue function.")
								end
							end
				...
				function Utilities:AddToggle(option)
					...
					Library.Flags[option.flag] = option.toggled
					Library.Items[option.flag] = option
	...
	function Library:CreateConfig()
		...
		for Index, Item in pairs(Library.Items) do
			local value
			...
			elseif Item.color then
				local r, g, b = Item.color.r, Item.color.g, Item.color.b
				local h, s, v = Color3.toHSV(Color3.new(r, g, b))
				value = {hue = h, color = {r, g, b}}
			else
				value = false
			end

			config[Item.flag] = value
		end
	...
	function Library:LoadConfig(name)
		...
		for Index, Item in pairs(Library.Items) do
			local value = cfg[Item.flag]

			if Item.utilitytype == "dropdown" then
				Item:SetValue(value)
			elseif Item.utilitytype == "keybind" then
				Item:SetValue(Enum.KeyCode[value])
			elseif Item.utilitytype == "toggle" then
				Item:SetValue(value or false)
			elseif Item.utilitytype == "slider" then
				Item:SetValue(value)
			elseif Item.utilitytype == "color" then
				Item:SetValue(value.hue, Color3.new(unpack(value.color)))
			end
		end
	end
	return TabHandler
